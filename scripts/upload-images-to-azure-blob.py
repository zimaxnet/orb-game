...
class AzureBlobImageUploader:
    def __init__(self):
        ...
        try:
            ...
            self.account_key = secret_client.get_secret("AZURE-STORAGE-ACCOUNT-KEY").value
            logging.info("✅ Retrieved storage account key from Key Vault")
        except Exception as e:
            logging.error(f"❌ Failed to get storage account key from Key Vault: {e}")
            logging.info("⚠️  Falling back to environment variable")
