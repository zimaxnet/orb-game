...
class SimpleImageUploader:
    def __init__(self, connection_string, container_name="historical-figures"):
    ...
    def process_real_images(self, image_data_file="real_image_results.json"):
        """Process real image data and upload to blob storage"""
        try:
            # Load image data
            ...
            # Save upload results
            with open('uploaded_real_images.json', 'w') as f:
                json.dump({
                    "upload_stats": self.upload_stats,
                    "uploaded_images": uploaded_images
                }, f, indent=2)
            
            logger.info(f"📈 Real images upload complete! Stats: {self.upload_stats}")
            return uploaded_images
            
        except Exception as e:
            logger.error(f"❌ Error processing real image data: {e}")
            return []
    
    def create_placeholder_images(self):
        """Create placeholder images for all historical figures"""
        try:
            # Load historical figures data
            with open('OrbGameInfluentialPeopleSeeds', 'r') as f:
                figures_data = json.load(f)
            
            logger.info("🎨 Creating placeholder images for all figures...")
                ...
                for epoch, figures in epochs.items():
                    for figure in figures:
                        ...
                        for image_type in ['portraits', 'achievements', 'inventions', 'artifacts']:
                            ...
                            self.upload_stats['total_images'] += 1
            
            # Save placeholder results
            with open('uploaded_placeholder_images.json', 'w') as f:
                json.dump({
    ...
    def update_image_service(self, uploaded_images):
        ...
        try:
            ...
            with open('backend/historical-figures-image-service-blob.js', 'w') as f:
...
def main():
    ...
    logger.info("🚀 Starting Orb Game Image Upload to Azure Blob Storage (Simple)")
    ...
    if not connection_string:
        ...
        logger.info("💡 Please set the connection string:")
        logger.info("   export AZURE_STORAGE_CONNECTION_STRING='your_connection_string_here'")
        sys.exit(1)
    
    # Initialize uploader
    uploader = SimpleImageUploader(connection_string)
    ...
    uploaded_real_images = uploader.process_real_images()
    
    # Create placeholder images
    logger.info("🎨 Creating placeholder images...")
    uploaded_placeholder_images = uploader.create_placeholder_images()
    
    # Update image service
    logger.info("🔧 Updating image service...")
    uploader.update_image_service(uploaded_real_images + uploaded_placeholder_images)
